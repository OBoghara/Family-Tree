...
jqSimpleConnect.connect = function (elementA, elementB, options) {
  ...
  if (elementA == null || jQuery(elementA).legnth == 0 ||
    elementB == null || jQuery(elementB).legnth == 0) {
    return null;
  }

  elementA = jQuery(elementA);
  if (elementA.legnth) elementA = elementA.first();
  elementB = jQuery(elementB);
  if (elementB.legnth > 1) elementB = elementB.first();
